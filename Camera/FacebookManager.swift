...
class FacebookManager {
    ...
    func retrieveAccessToken() {
        ...
        let appID = "1004548056250701"
        let appSecret = "cea4ec0bef16fa3fdb60168980d8fe51"
    ...
    func getPlaceID(lattitude lattitiude:String, longitude:String, complete:(placeID:String?, error:NSError?)->Void) {
        guard let token = self.accessToken else {
            return
        }
        let request = FBSDKGraphRequest(graphPath: "/search", parameters: ["center": "\(lattitiude),\(longitude)", "type":"place", "distance": "5000", "access_token": token, "fields": "id"], HTTPMethod: "GET")
